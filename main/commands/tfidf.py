...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python tfidf.py [btts|double_chance|three_way] [live|upcoming]")
        sys.exit(1)
    ...
    try:
        ...
        json_event_stream = fetch_data(stream_name)
        ...
        for group in groups.values():
            ...
            # After populating this mapping, you can then check if any team names meet your threshold of bookmaker listings before proceeding to save any data to Redis.

            if len(team_names) >= least_count:
                # redis_db.json().set(match_key, Path.root_path(), {})
                ...
                print(
                    f"Updated match info in Redis under key {match_key} with teams {team_names_str} with match_id {match_id}"
                )
